...
/**
 * 快速生成适配dimens工具类
 * 将该文件放到项目的包下，在项目发布之前打开该文件，运行一次DimenTool.main()即可
 * 以sw320dp为标准  即values文件下的dimens
 *
 * @author  wujiuye
 * 该类需要使用Terminal编译运行，否则会报错
 * cd到java目录下，然后javac -d . wjy/weiai/DimenUtil.java
 * 运行(在java目录下): java wjy.weiai.DimenUtil
 */
public class DimenUtil {
    ...
    public static void gen() {
        ...
        String rootPath = filePath.substring(0,filePath.lastIndexOf("java"));
        ...
        try {
            ...
            while ((tempString = reader.readLine()) != null) {
                if (tempString.contains("</dimen>")) {
                    ...
                    }else if(subStr.indexOf("dp")>0){
                        danwei="dp";
                        subStr = subStr.substring(0, subStr.length()-2);
                    }else if(subStr.indexOf("sp")>0){
                        danwei="sp";
                        subStr = subStr.substring(0,subStr.length()-2);
                    }
                    ...
                    sw240.append(start).append( num * 0.75).append(danwei).append(end).append("\r\n");
                    sw360.append(start).append( num * 1.125).append(danwei).append(end).append("\r\n");
                    sw480.append(start).append(num * 1.5).append(danwei).append(end).append("\r\n");
                    sw600.append(start).append(num * 1.87).append(danwei).append(end).append("\r\n");
                    sw720.append(start).append(num * 2.25).append(danwei).append(end).append("\r\n");
                    sw800.append(start).append(num * 2.5).append(danwei).append(end).append("\r\n");
                    w820.append(start).append(num * 2.56).append(danwei).append(end).append("\r\n");
                } else {
