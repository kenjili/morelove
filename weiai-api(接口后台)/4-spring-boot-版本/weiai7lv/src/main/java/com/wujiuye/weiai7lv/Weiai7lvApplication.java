...
import com.wujiuye.weiai7lv.controller.FoundController;
import com.wujiuye.weiai7lv.controller.IndexController;
...
@Slf4j
public class Weiai7lvApplication {

    public static void main(String[] argv) {
        log.debug("jar方式启动spring boot项目...");
        ApplicationContext applicationContext = SpringApplication.run(Weiai7lvApplication.class, argv);
        /**
         * 处理器方法
         * 知识点补充：
        ...
        try {
            ...
            Method indexMethod = indexController.getClass()
                    //方法名，方法参数类型
                    .getMethod("index", null/*String.class, Integer.class, Integer.class, Integer.class, Integer.class*/);
            ...
            if (indexMethod.getReturnType() == WebResult.class) {
                //调用目标bean的目标方法
                WebResult result = (WebResult) handlerMethod.getMethod().invoke(handlerMethod.getBean(), null/*"广州", null, null, null, null*/);
                System.out.println("result===>" + result.getErrorCode() + "," + result.getErrorMessage() + "," + result.getData());
            }
