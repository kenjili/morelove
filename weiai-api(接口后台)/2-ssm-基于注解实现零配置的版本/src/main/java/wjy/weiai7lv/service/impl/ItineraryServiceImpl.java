...
import wjy.weiai7lv.dao.*;
import wjy.weiai7lv.entity.*;
...
public class ItineraryServiceImpl implements ItineraryService {
    ...
    public Itinerary publicItinerary(Itinerary itinerary, Album album, MultipartFile[] photoFiles) {
        ...
        String loverAlbumPath = "/lover_" + lover.getId() + "/album_" + album.getId();
        //一张张图片上传到相册目录下
        for (MultipartFile photo : photoFiles
            ...
            String photoPath = loverAlbumPath + "/photos_" + photos.getId();
            //获取图片保存的相对路径【原图和缩略图】，=情侣相册路径/图片id
            String[] relativePathAndthumbImagePath = UploadImageFileUtils.savaImageFile(photo, photoPath, UploadImageFileUtils.UploadImageType.ALBUM);
            //更新图片路径信息
        ...
        return itineraryDao.queryItinerary(itinerary.getId());
    }
    ...
    public List<Itinerary> listItinerary(int pageSize, int currentPage) {
        ...
        for (Itinerary itinerary : list) {
            for (Photos photo : itinerary.getAlbum().getPhotosList()) {
                //修改图片路径为详细的网路访问路径
                photo.setImg(WebConstantConfig.getAlbumResourceServerName() + photo.getImg());
                photo.setImgThumb(WebConstantConfig.getAlbumResourceServerName() + photo.getImgThumb());
            }
        }
        return list;
    }
